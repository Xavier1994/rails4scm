module Calendar
    module Helper
        def calendar_field(object, field,value=nil,sn=nil, options={})
            ...
            if sn == nil then
              textname= object.to_s + "[" + field.to_s + "]"
            else 
              textname= object.to_s + "[" + sn.to_s + "][" + field.to_s + "]"
            end
            <<-EOL
          ...
          <input type='text' name='#{textname}' size=#{size} id='#{object}_#{field}' value='#{value}'>

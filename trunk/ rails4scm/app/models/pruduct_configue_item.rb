class PruductConfigueItem < ActiveRecord::Base
  ...
  def getProductConfigueItems(pageSize,curPageSize,product,productVer)
    sql = "SELECT CONFIGURE_ITEM.ID,SOFTWARE_PRODUCT.PRODUCT_NAME,SOFTWARE_PRODUCT_VER.PRODUCT_CONF_PERSON,CONFIGURE_ITEM.CONFIGURE_CODE,CONFIGURE_ITEM.CONFIGURE_NAME,CONFIGURE_VERSION.CONFIGURE_VERS,CONFIGURE_ITEM.CONFIGURE_TYPE,CONFIGURE_ITEM.CONFIGURE_STORE_ADDR,CONFIGURE_VERSION.STATE_DATE,CONFIGURE_VERSION.CUR_STATE, PRODUCT_COMP_CONF.PRODUCT_CODE,SOFTWARE_PRODUCT_VER.PRODUCT_VERS "
  ...
  def  productVerProjects(pageSize,curPageSize,product_code,product_ver)
    ...
    PruductConfigueItem.paginate_by_sql([sql],:per_page =>pageSize,:page =>curPageSize,:order=>"PROJECT_CODE")
  end
end

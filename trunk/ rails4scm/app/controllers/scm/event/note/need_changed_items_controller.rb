class Scm::Event::Note::NeedChangedItemsController < ApplicationController
  ...
  def changeItem
    ...
    outText="<table cellpadding=0 cellspacing=0 border=0 style=\"width:100%;bordercolo:#878787 ;border-left-style: solid; border-left-width: 1px; border-right-style: solid; border-right-width: 1px\">"
    @selectedItems=nil
    if configure_params!=nil then
      ...
      if event_code!=nil && configureParamsArray[0]!=nil && configureParamsArray[2]!=nil then
        ...
        configureItems=configureItem.findEventConfigureItemSelected(event_code)
          if configureItems!=nil && configureItems.size>0 then
            for configureitem in configureItems
              outText+="<tr>"
              outText+="<td class='td6' width='150px'>&nbsp;"+configureitem.CONFIGURE_NAME+"</td>"
              outText+="<td class='td7' width='80px'>&nbsp;"+configureitem.CONFIGURE_VERS+"</td>"
